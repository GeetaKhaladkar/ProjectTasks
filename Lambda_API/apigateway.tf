resource "aws_api_gateway_rest_api" "terraform_lambda_func" {
  name        = "Serverlessterraform_lambda_func"
...
resource "aws_api_gateway_method" "number_rootpy" {
   rest_api_id   = aws_api_gateway_rest_api.terraform_lambda_func.id
   resource_id   = aws_api_gateway_rest_api.terraform_lambda_func.root_resource_id
   http_method   = "GET"
   authorization = "NONE"
}
...
resource "aws_api_gateway_deployment" "terraform_lambda_func" {
   depends_on = [
     aws_api_gateway_integration.lambdapy,
...
output "base_url" {
  value = "${aws_api_gateway_deployment.terraform_lambda_func.invoke_url}/${var.resource_name}"
}
